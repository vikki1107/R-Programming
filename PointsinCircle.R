...
# For ex: for an input arg pts, the function will first check if its a numeric matrix by checking the numeric, class       #
# and then checks if the matrix has atleast one row and two column. It also checks for any NA or NAN values present        #
# Similarly it also checks if the center is just one point with one xy coordinate and a radius is numeric value.           #
#                                                                                                                          #
...
{ # start function in.circle
  ...
  if ( !(is.numeric(pts) && class(pts) == "matrix" && nrow(pts) >= 1 && ncol(pts) == 2) ) 
  { stop("Either 1st argument/pts is not a numeric matrix or it is not of dimension nX2") }
  if ( (any(is.na(pts)) || any(is.nan(pts))) ) 
  { stop("Either na or nan value detected in a matrix") }
  if ( !(is.numeric(cntr) && length(cntr) == 2) ) 
  { stop("Either the 2nd argument/center is not numeric or it is not a vector of length 2") }
  if ( (any(is.na(cntr)) || any(is.nan(cntr))) ) 
  { stop("Either na or nan value detected in vector cntr") }
  if ( !(is.numeric(r) && r > 0 && length(r) == 1) ) 
  { stop("Either the 3rd argument/radius is not numeric or it is not greater than 0 or it is a vector of length greater than 1") }
